...
public class MeleeDamageUpgrade : Upgrade
{
    ...
    {
        float upgAmount = 0.15f * (upgradeManager.playerStatsUpgLevels[PlayerStatsUpgrade.MeleeDamage] + 2) *
    ...
    public override void Apply(PlayerStats playerStats, int level)
    {
        playerStats.meleeDamage += 0.15f * (level + 1) * PlanetManager.Instance.statMultipliers[PlanetStatModifier.LaserDamage];
    }
...
public class SellAttackDroneUpgrade : Upgrade
{
    public SellAttackDroneUpgrade() : base(ConsumablesUpgrade.SellAttackDrone) { }

    public static int Amount = 1;

    public override bool Apply(PlayerStats playerStats)
    {
        return DroneManager.Instance.SellDrone(DroneType.Attacker, playerStats.structureStats.currentDroneContainer);
    }
