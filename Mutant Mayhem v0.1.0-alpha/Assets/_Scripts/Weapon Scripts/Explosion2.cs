...
public class Explosion2 : MonoBehaviour
{
    ...
    void Explode(Vector2 explosionPos, List<Vector3Int> tilesToCheck, TileManager tileManager)
    {
        ...
        foreach (Vector3Int tilePos in tilesToCheck)
        {
            ...
            RaycastHit2D hit = Physics2D.Raycast(explosionPos, dirToTile, Vector2.Distance(tileCenter, explosionPos), LayerMask.GetMask("Structures"));
            ...
            if (hit.collider != null)
            {
                ...
                {
                    // Apply damage to the structure if it hasn’t been hit yet
    ...
    void ApplyDamageToEntitiesInTiles(Vector2 explosionPos, List<Vector3Int> visibleTiles)
    {
        foreach (Vector3Int tilePos in visibleTiles)
        {
            ...
            Collider2D[] entitiesInTile = Physics2D.OverlapBoxAll(tileCenter, tileSize, 0, LayerMask.GetMask("Enemies", "Player"));
            ...
            foreach (Collider2D entity in entitiesInTile)
            {
                ...
                if (player != null)
                {
                    ...
                    float totalDamage = Mathf.Clamp(damage / distToPoint, 0, damage / 2); // Maybe less damage to the player
